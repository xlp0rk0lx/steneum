...
#if defined(NDEBUG)
# error "Steneum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6786546873c422dfc3780ea05ebce6b6ef8056ec77541c66f6336d7d64cf69bf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Steneum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Steneum Signed Message:\n";
...
{
    // Steneum: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Steneum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Steneum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1510818018, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Wallet online December 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04994789cdabafc947af39eff4a8ca8f72124404324bf89adece7595aede3751f609234c3d14a3504a0d6e741e9d2c313c5c0a02f0360a2307b6fb7566c52b82c4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1510818018;
        ...
        assert(block.hashMerkleRoot == uint256("0x74eba009a5abdb990a3bb309c3833520cfdccef2e5ab6a34f394bea7cc867c7c"));
...
unsigned char pchMessageStart[4] = { 0xe6, 0x26, 0x17, 0x0d }; // Steneum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SteneumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SteneumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SteneumMiner(CWallet *pwallet)
{
    printf("SteneumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("steneum-miner");
    ...
    try { loop {
        ...
        printf("Running SteneumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SteneumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SteneumMiner, pwallet));
}
